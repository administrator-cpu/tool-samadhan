export class EmployeeRepository {
    static async create(client, userId) {
        const result = await client.query(`INSERT INTO employees (user_id)
       VALUES ($1)
       RETURNING id, employee_id, joined_at`, [userId]);
        return result.rows[0];
    }
    static async findByUserId(client, userId) {
        const result = await client.query(`SELECT e.id, e.employee_id, u.role, u.name
       FROM employees e
    ...
    static async findByRowId(client, employeeRowId) {
        const result = await client.query(`SELECT user_id FROM employees WHERE id = $1 LIMIT 1`, [employeeRowId]);
        return result.rowCount && result.rowCount > 0 ? result.rows[0] : null;
    }
    static async findAllWithCategories(pool) {
    ...
    static async findAllAgents(pool) {
        const result = await pool.query(`
    ...
    static async replaceCategoriesByName(client, employeeRowId, categoryNames) {
        await client.query(`DELETE FROM employee_issue_categories WHERE employee_id = $1`, [employeeRowId]);
        if (categoryNames && categoryNames.length > 0) {
            await client.query(`INSERT INTO employee_issue_categories (employee_id, issue_category_id)
         SELECT $1, id FROM issue_categories WHERE name = ANY($2)`, [employeeRowId, categoryNames]);
        }
    }
    static async findBestAgentForCategory(client, categoryId) {
        if (!categoryId)
            return null;
