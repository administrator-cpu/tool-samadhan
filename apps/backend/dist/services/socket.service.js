    ...
    io.on('connection', (socket) => {
        ...
        socket.on('sync_missed_events', async ({ ticketId, lastSeenEventId }) => {
            try {
                ...
                const result = await postgresPool.query(`SELECT te.id, te.ticket_id, te.actor_user_id, u.name AS actor_name, 
           ...
           ORDER BY te.id ASC
           LIMIT 201`, [ticketId, lastSeenEventId]);
...
const checkTicketAccess = async (userId, ticketId) => {
    ...
    try {
        ...
        if (role === 'SUPPORT_AGENT') {
            const agentRes = await client.query('SELECT 1 FROM tickets t JOIN employees e ON e.id = t.current_assigned_employee_id WHERE t.id = $1 AND e.user_id = $2', [ticketId, userId]);
            return agentRes.rowCount !== null && agentRes.rowCount > 0;
        }
        const customerRes = await client.query('SELECT 1 FROM tickets t JOIN customers c ON c.id = t.customer_id WHERE t.id = $1 AND c.user_id = $2', [ticketId, userId]);
