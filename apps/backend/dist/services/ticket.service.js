...
export class TicketService {
    static async createTicket(dto, actorUserId, actorRole) {
        return withTransaction(async (client) => {
            let customerId = null;
            if (actorRole === UserRole.USER) {
                const cust = await CustomerRepository.findByUserId(client, actorUserId);
                if (!cust)
                    throw new AppError(404, 'Customer record not found', ErrorCodes.CUSTOMER_NOT_FOUND);
                customerId = cust.id;
            }
            else {
            ...
            if (!customerId)
                throw new AppError(400, 'Customer ID is required', ErrorCodes.VALIDATION_ERROR);
            ...
            if (info) {
                if (!assignedAgentId) {
                    ...
                    await ticketAutomationQueue.add('AGENT_ASSIGNMENT_CHECK', { ticketId: ticket.id }, { delay: 2 * 60 * 1000 });
                }
                else {
    ...
    static async listUserTickets(userId, role, page, limit, filters) {
        const client = await postgresPool.connect();
        try {
            const offset = (page - 1) * limit;
            const queryFilters = { statusGroup: filters.statusGroup };
            if (role === UserRole.USER) {
                const cust = await CustomerRepository.findByUserId(client, userId);
                if (!cust)
                    return { tickets: [], total: 0, pages: 0, currentPage: page, limit };
            ...
            if (['ADMIN', 'MANAGER'].includes(role)) {
                if (filters.ownership)
                    queryFilters.ownership = filters.ownership;
                if (filters.agentId)
                    queryFilters.employeeId = filters.agentId;
            }
    ...
    static async getTicketTimeline(ticketId, userId, role, afterEventId, limit = 200) {
        const client = await postgresPool.connect();
        try {
            const ticketInfo = await TicketRepository.findTicketTimelineInfo(client, ticketId);
            if (!ticketInfo)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
    ...
    static async addTicketEvent(ticketId, dto, actorUserId, role) {
        return withTransaction(async (client) => {
    ...
    static async updateTicketStatus(ticketId, dto, actorUserId, role) {
        return withTransaction(async (client) => {
            const ticket = await TicketRepository.findByIdForUpdate(client, ticketId);
            if (!ticket)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
            ...
            if (updatedStatus === 'RESOLVED') {
                await ticketAutomationQueue.add('CLOSE_RESOLVED_TICKET', { ticketId }, { delay: 24 * 60 * 60 * 1000, jobId: `close-resolved-${ticketId}` });
            }
            ticketEventEmitter.emit('ticket_updated', {
                ticketId,
    ...
    static async updateTicketRca(ticketId, rca, actorUserId) {
        return withTransaction(async (client) => {
            const ticket = await TicketRepository.findByIdForUpdate(client, ticketId);
            if (!ticket)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
    ...
    static async updateOutageDetails(ticketId, dto, actorUserId) {
        return withTransaction(async (client) => {
            const ticket = await TicketRepository.findByIdForUpdate(client, ticketId);
            if (!ticket)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
    ...
    static async reassignTicket(ticketId, employeeId, actorUserId) {
        return withTransaction(async (client) => {
            const ticket = await TicketRepository.findByIdForUpdate(client, ticketId);
            if (!ticket)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
    ...
    static async toggleCustomerReply(ticketId, allow, actorUserId) {
        return withTransaction(async (client) => {
            const ticket = await TicketRepository.findByIdForUpdate(client, ticketId);
            if (!ticket)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
    ...
    static async listResolvedTickets(page, limit, exportAll = false) {
        const client = await postgresPool.connect();
        try {
            const offset = (page - 1) * limit;
    ...
    static async updateTicketRating(ticketId, rating, feedback, actorUserId) {
        return withTransaction(async (client) => {
            const ticket = await TicketRepository.findByIdForUpdate(client, ticketId);
            if (!ticket)
                throw new AppError(404, 'Ticket not found', ErrorCodes.TICKET_NOT_FOUND);
