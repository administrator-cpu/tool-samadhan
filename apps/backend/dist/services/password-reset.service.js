...
export class PasswordResetService {
    ...
    static async completePasswordReset(email, otpCode, newPassword) {
        return withTransaction(async (client) => {
            const otpRecord = await PasswordResetRepository.verifyOtp(client, email, otpCode);
            if (!otpRecord) {
                throw new AppError(400, 'Invalid or expired OTP', ErrorCodes.INVALID_OTP);
            ...
            await client.query(`UPDATE sessions SET revoked = TRUE, revoked_at = NOW() WHERE user_id = $1`, [otpRecord.user_id]);
