...
export class UserService {
    ...
    static async listAllCustomers(page, limit) {
    ...
    static async updateEmployee(employeeRowId, dto) {
        return withTransaction(async (client) => {
            const empInfo = await EmployeeRepository.findByRowId(client, employeeRowId);
            if (!empInfo)
                throw new AppError(404, 'Employee not found', ErrorCodes.EMPLOYEE_NOT_FOUND);
    ...
    static async deleteEmployee(employeeRowId) {
        return withTransaction(async (client) => {
            const empInfo = await EmployeeRepository.findByRowId(client, employeeRowId);
            if (!empInfo)
                throw new AppError(404, 'Employee not found', ErrorCodes.EMPLOYEE_NOT_FOUND);
    ...
    static async deleteCustomer(customerRowId) {
        return withTransaction(async (client) => {
            const custInfo = await CustomerRepository.findByRowId(client, customerRowId);
            if (!custInfo)
                throw new AppError(404, 'Customer not found', ErrorCodes.CUSTOMER_NOT_FOUND);
    ...
    static async updateUserProfile(userId, dto) {
        return withTransaction(async (client) => {
            if (dto.email) {
                const userCheck = await UserRepository.findByEmail(client, dto.email);
                if (userCheck && userCheck.id !== userId) {
    ...
    static async getCurrentUserDetails(userId) {
        ...
        try {
            ...
            if (user.role === UserRole.USER) {
                ...
                if (user.role === UserRole.SUPPORT_AGENT && emp) {
                    const catRes = await client.query(`SELECT ic.name FROM employee_issue_categories eic 
             JOIN issue_categories ic ON eic.issue_category_id = ic.id 
             WHERE eic.employee_id = $1`, [emp.id]);
