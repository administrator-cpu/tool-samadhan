        ...
        <header className="mb-10 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-white shadow-lg shadow-[#F5821F]/20">
                <Target size={20} />
              </div>
              <div>
            ...
            <div className="rounded-[1rem] border border-slate-100 bg-white p-8 shadow-xl shadow-slate-200/30">
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-black text-primary">{Math.min(100, Math.round((Number(summary.resolved_today) / 10) * 100))}%</span>
                </div>
...
function StatCard({ icon, label, value, sublabel, color, bg, alert, highlight }: any) {
  return (
    <div className={`group relative overflow-hidden rounded-[1rem] border ${alert ? 'border-red-200 shadow-red-100' : highlight ? 'border-primary/20 shadow-[#F5821F]/10' : 'border-slate-100'} bg-white p-7 shadow-xl shadow-slate-200/40 transition-all hover:-translate-y-1 hover:shadow-2xl`}>
      <div className="flex flex-col gap-4">
        ...
        <div className="pt-2 border-t border-slate-50 mt-1">
          <span className={`text-[10px] font-black uppercase tracking-widest ${alert ? 'text-red-500' : highlight ? 'text-primary' : 'text-slate-400'}`}>{sublabel}</span>
        </div>
      </div>
    </div>
  );
