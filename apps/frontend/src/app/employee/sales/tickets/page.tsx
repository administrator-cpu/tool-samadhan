...
const statusColors: Record<string, string> = {
  OPEN: "bg-[#F5821F]/10 text-primary",
            ...
            <div className="flex h-12 items-center rounded-lg border border-slate-200 bg-white px-2 py-1 gap-1">
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 rounded-md px-4 py-2 text-xs font-bold transition-all ${statusFilter !== "ALL" ? "bg-brand-gradient text-white shadow-lg border-none" : "text-slate-500 hover:bg-slate-50"}`}
                >
                  <Filter size={14} />
                    ...
                    <div className="absolute top-full left-0 mt-2 w-48 rounded-lg border border-slate-200 bg-white shadow-xl z-50 overflow-hidden">
                      {["ALL", "OPEN", "IN_PROGRESS", "ESCALATED", "RESOLVED", "CLOSED"].map((status) => (
                        <button
                          ...
                          className={`block w-full text-left px-4 py-2 text-xs font-bold transition-colors ${statusFilter === status ? "bg-[#F5821F]/10 text-primary" : "text-slate-600 hover:bg-[#F5821F]/5 hover:text-slate-900"}`}
                        >
              ...
              <button
                ...
                className={`flex items-center gap-2 rounded-md px-4 py-2 text-xs font-bold transition-all ${sortField === "date" ? "bg-brand-gradient text-white shadow-lg border-none" : "text-slate-500 hover:bg-slate-50"}`}
              >
                <Calendar size={14} />
                Date
                    ...
                    <tr key={ticket.id} className="group transition-all hover:bg-slate-50/80">
                      ...
                      <td className="px-8 py-6 text-right">
                        <Link
                          ...
                          className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 transition-all hover:bg-[#F5821F]/10 hover:text-primary hover:border-primary hover:-translate-y-0.5 active:scale-95"
                        >
